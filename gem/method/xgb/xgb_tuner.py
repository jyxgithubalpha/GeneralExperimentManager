"""
XGBoostTuner - XGBoost Optuna 参数调优器
"""
...
class XGBoostTuner(BaseTuner):
    ...
    def tune(
        ...
        try:
            ...
            raise ImportError("optuna is required for XGBoostTuner")
        ...
        def objective(trial) -> float:
            ...
            # 获取验证集指标
            metric_key = "rmse"  # XGBoost 默认指标
            if "val" in fit_result.evals_result:
                for key in fit_result.evals_result["val"]:
                    scores = fit_result.evals_result["val"][key]
                    if fit_result.best_iteration and fit_result.best_iteration <= len(scores):
