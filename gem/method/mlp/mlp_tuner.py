...
class MLPTuner(BaseTuner):
    ...
    def tune(
        ...
        try:
            ...
            raise ImportError("optuna is required for MLPTuner")
        ...
        def objective(trial) -> float:
            ...
            # 获取验证集损失
            if "val" in fit_result.evals_result and "loss" in fit_result.evals_result["val"]:
                losses = fit_result.evals_result["val"]["loss"]
                if losses:
                    # 返回负的损失，因为我们要最小化损失但 direction 是 maximize
                    return -min(losses)
            return float('-inf')
